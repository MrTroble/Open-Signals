...
public class FunctionParsingInfo {
	...
	public FunctionParsingInfo(final Map<String, MethodInfo> translationTable, final Signal signal) {
		this(signal);
	...
	public Object getProperty(final String propertyName) {
		...
		final SEProperty property = propertyCache.computeIfAbsent(name, _u -> {
			return properties.stream().filter(noneCache -> noneCache.getName().equalsIgnoreCase(name)).findAny()
					.orElse(null);
		});
		if (property == null) {
	...
	public Object getPredicate() {
		final ValuePack predicate = predicateCache.computeIfAbsent(argument.toLowerCase(), _u -> {
			...
			if (parts.length != 2)
				throw new LogicalParserException(String
						.format("Syntax error predicate need to have the form PROPERTY.NAME but was %s", argument));
			argument = parts[0];
		...
		if (predicate == null)
			throw new LogicalParserException(
					String.format("Could not make predicate=%s with system=%S!", argument, signalName));
		return predicate;
	}
	...
	public Object getStringInt() {
		final StringInteger strInt = strIntCache.computeIfAbsent(argument.toLowerCase(), _u -> {
			...
			if (parts.length != 2)
				throw new LogicalParserException(String
						.format("Syntax error speedValue need to have the form PROPERTY.NAME but was %s", argument));

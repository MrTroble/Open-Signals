...
public class SignalBoxGrid implements INetworkSavable {
	...
	private boolean checkPathwayData(final PathwayData data) {
		return startsToPath.containsKey(data.getFirstPoint()) || endsToPath.containsKey(data.getLastPoint());
	}
	...
	public void resetAllSignals() {
		...
		final Map<Point, Map<ModeSet, SubsidiaryEntry>> copy = ImmutableMap.copyOf(enabledSubsidiaryTypes);
		copy.forEach((point, map) -> {
			final Map<ModeSet, SubsidiaryEntry> entryCopy = ImmutableMap.copyOf(map);
			entryCopy.forEach(
					(mode, entry) -> updateSubsidiarySignal(point, mode, new SubsidiaryEntry(entry.enumValue, false)));
		});
	}
	...
	public List<Point> getAllInConnections() {
		return modeGrid.values().stream().filter(SignalBoxNode::containsInConnection).map(SignalBoxNode::getPoint)
				.collect(Collectors.toList());
	}

	public List<Point> getValidEnds() {
		return modeGrid.values().stream().filter(SignalBoxNode::isValidEnd).map(SignalBoxNode::getPoint)
				.collect(Collectors.toList());
	}
	...
	public void write(final NBTWrapper tag) {
		tag.putList(NODE_LIST, modeGrid.values().stream().filter(node -> !node.isEmpty()).map(node -> {
			final NBTWrapper nodeTag = new NBTWrapper();
			node.write(nodeTag);
			final Map<ModeSet, SubsidiaryEntry> subsidiaries = enabledSubsidiaryTypes.get(node.getPoint());
		...
		if (tag.contains(NEXT_PATHWAYS)) {
			tag.getList(NEXT_PATHWAYS).forEach(comp -> {
				...
				if (comp.contains(PATH_TYPE)) {
					nextPathways.put(Maps.immutableEntry(start, end), PathType.valueOf(comp.getString(PATH_TYPE)));
				} else {
	...
	public String toString() {
		return "SignalBoxGrid [modeGrid="
				+ modeGrid.entrySet().stream().map(entry -> entry.toString()).collect(Collectors.joining("\n")) + "]";
	}
	...
	public void readNetwork(final ReadBuffer buffer) {
		...
		for (int i = 0; i < size; i++) {
			...
			if (enabledSubsidariesSize != 0) {
				for (int j = 0; j < enabledSubsidariesSize; j++) {
					final Map<ModeSet, SubsidiaryEntry> allTypes = enabledSubsidiaryTypes.computeIfAbsent(point,
							_u -> new HashMap<>());
		...
		for (int i = 0; i < greenSignalsSize; i++) {
			...
			final Map<ModeSet, SubsidiaryEntry> subsidiary = enabledSubsidiaryTypes
					.computeIfAbsent(identifier.getPoint(), _u -> new HashMap<>());
			final SubsidiaryEntry entry = subsidiary.get(identifier.getModeSet());
			if (entry != null) {
				identifier.state = SignalState.combine(entry.enumValue.getSubsidiaryShowType());
			}
			modeGrid.computeIfPresent(identifier.getPoint(), (p, n) -> {
				n.updateState(identifier.getModeSet(), identifier.state);
				return n;
			});
		}
	}
	...
	public void writeNetwork(final WriteBuffer buffer) {
		...
		modeGrid.forEach((point, node) -> {
			...
			final Map<ModeSet, SubsidiaryEntry> enabledSubsidiaries = enabledSubsidiaryTypes.computeIfAbsent(point,
					_u -> Maps.newHashMap());
		...
		List<MainSignalIdentifier> greenSignals = getGreenSignals();
	...
	public BlockPos updateManuellRSOutput(final Point point, final ModeSet mode, final boolean state) {
		...
		final EnumPathUsage usage = entry.getEntry(PathEntryType.PATHUSAGE).orElse(EnumPathUsage.FREE);
	...
	public void updateSubsidiarySignal(final Point point, final ModeSet mode, final SubsidiaryEntry entry) {
		final SignalBoxNode node = modeGrid.get(point);
		if (node == null)
			return;
		...
		if (!pos.isPresent())
			return;
		final Signal signal = SignalBoxHandler.getSignal(new StateInfo(tile.getLevel(), tile.getBlockPos()), pos.get());
		if (!entry.state) {
			final Map<ModeSet, SubsidiaryEntry> states = enabledSubsidiaryTypes.get(point);
			if (states == null || !states.containsKey(mode))
				return;
			SignalConfig.reset(new ResetInfo(new SignalStateInfo(tile.getLevel(), pos.get(), signal), false));
			states.remove(mode);
			if (states.isEmpty()) {
		...
		final Map<ModeSet, SubsidiaryEntry> states = enabledSubsidiaryTypes.computeIfAbsent(point,
				_u -> new HashMap<>());
		final Map<SubsidiaryState, ConfigProperty> configs = SubsidiarySignalParser.SUBSIDIARY_SIGNALS.get(signal);
		if (configs == null)
			return;
	...
	public boolean getSubsidiaryState(final Point point, final ModeSet mode, final SubsidiaryState type) {
		final Map<ModeSet, SubsidiaryEntry> states = enabledSubsidiaryTypes.get(point);
		if (states == null)
			return false;

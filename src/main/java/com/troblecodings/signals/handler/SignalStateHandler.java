...
public final class SignalStateHandler implements INetworkSync {
	...
	public static void createStates(final SignalStateInfo info, final Map<SEProperty, String> states) {
	...
	public static void addListener(final SignalStateInfo info, final SignalStateListener listener) {
		synchronized (ALL_LISTENERS) {
			final List<SignalStateListener> listeners = ALL_LISTENERS.computeIfAbsent(info, _u -> new ArrayList<>());
			listeners.add(listener);
		}
	}

	public static void removeListener(final SignalStateInfo info, final SignalStateListener listener) {
	...
	private static void statesToBuffer(final Signal signal, final Map<SEProperty, String> states,
		...
		states.forEach((property, string) -> {
			readData[signal.getIDFromProperty(property)] = (byte) (property.getParent().getIDFromValue(string) + 1);
		});
	}

	private static void createToFile(final SignalStateInfo info, final Map<SEProperty, String> states) {
		if (states == null)
			return;
	...
	public static void setStates(final SignalStateInfo info, final Map<SEProperty, String> states) {
		...
		synchronized (CURRENTLY_LOADED_STATES) {
			if (CURRENTLY_LOADED_STATES.containsKey(info)) {
				contains.set(true);
				final Map<SEProperty, String> oldStates = new HashMap<>(CURRENTLY_LOADED_STATES.get(info));
	...
	public static void setState(final SignalStateInfo info, final SEProperty property, final String value) {
	...
	private static void sendToPlayer(final SignalStateInfo stateInfo, final Map<SEProperty, String> properties,
	...
	private static void sendToAll(final SignalStateInfo stateInfo, final Map<SEProperty, String> properties) {
		if (properties == null || properties.isEmpty()) {
			return;
		}
	...
	public static void onChunkWatch(final ChunkWatchEvent.Watch event) {
			...
			if (!ALL_LEVEL_FILES.containsKey(world)) {
				ALL_LEVEL_FILES.put(world,
						new SignalStateFile(Paths.get("osfiles/signalfiles/"
								+ ((WorldServer) world).getMinecraftServer().getName().replace(":", "").replace("/", "")
										.replace("\\", "")
		...
		chunk.getTileEntityMap().forEach((pos, tile) -> {
			if (tile instanceof SignalTileEntity) {
				...
				final SignalStateInfo info = new SignalStateInfo(world, pos, signalTile.getSignal());
				states.add(info);
				synchronized (CURRENTLY_LOADED_STATES) {
					if (CURRENTLY_LOADED_STATES.containsKey(info)) {
	...
	public static void loadSignals(final List<SignalStateInfo> signals, final @Nullable EntityPlayer player) {
			...
			signals.forEach(info -> {
				synchronized (ALL_LEVEL_FILES) {
					if (!ALL_LEVEL_FILES.containsKey(info.world)) {
						ALL_LEVEL_FILES.put(info.world, new SignalStateFile(Paths.get("osfiles/signalfiles/"
								+ ((WorldServer) info.world).getMinecraftServer().getName().replace(":", "")
										.replace("/", "").replace("\\", "")
								+ "/"
								+ ((WorldServer) info.world).provider.getDimensionType().getName().replace(":", ""))));
					}
				}
				synchronized (SIGNAL_COUNTER) {

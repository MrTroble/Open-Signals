...
public class GuiRedstoneIO extends GuiBase {
	...
	private void init() {
		...
		textField.add(input);
		hbox.add(textField);
		final UIEntity apply = GuiElements.createButton(I18n.format("btn.apply"), _u -> this.updateText(input.getText()));
		...
		list.add(layout);
		this.tile.forEach(pos -> list.add(GuiElements.createLabel(String.format("%s: x=%d, y=%d, z=%d", GIRBlocks.SIGNAL_BOX.getLocalizedName(), pos.getX(), pos.getY(), pos.getZ()))));
		inner.add(list);

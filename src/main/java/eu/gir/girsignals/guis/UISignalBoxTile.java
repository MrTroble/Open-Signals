...
public class UISignalBoxTile extends UIComponent implements UIAutoSync {
	...
	public static final ResourceLocation ICON = new ResourceLocation(GirsignalsMain.MODID, "gui/textures/symbols.png");
	...
	public synchronized void draw(final int mouseX, final int mouseY) {
		...
		node.forEach((modeSet) -> {
			final EnumPathUsage usage = node.getOption(modeSet).map(entry -> entry.getEntry(PathEntryType.PATHUSAGE).orElse(EnumPathUsage.FREE)).orElse(EnumPathUsage.FREE);

...
import eu.gir.guilib.ecs.entitys.UIIndependentTranslate;
import eu.gir.guilib.ecs.entitys.UILabel;
import eu.gir.guilib.ecs.entitys.UIRotate;
import eu.gir.guilib.ecs.entitys.UIScale;
import eu.gir.guilib.ecs.entitys.UIScissor;
import eu.gir.guilib.ecs.entitys.UITexture;
...
public class GuiSignalController extends GuiBase {
	...
	private void createPageForSide(EnumFacing face, UIEntity leftSide, UIBlockRender bRender) {
		...
		final IIntegerable<String> profile = SizeIntegerables.of("profile", 32, in -> String.valueOf(in));
		...
		for (SEProperty<?> entry : map.keySet()) {
			if ((entry.isChangabelAtStage(ChangeableStage.APISTAGE) || entry.isChangabelAtStage(ChangeableStage.APISTAGE_NONE_CONFIG)) && entry.test(map.entrySet())) {
				final UIEntity entity = GuiElements.createEnumElement(new DisableIntegerable<>(entry), e -> applyModelChange(bRender));
		...
		final IIntegerable<Object> offProfile = new DisableIntegerable(SizeIntegerables.of("profileOff." + face.getName(), 32, in -> String.valueOf(in)));
		final IIntegerable<Object> onProfile = new DisableIntegerable(SizeIntegerables.of("profileOn." + face.getName(), 32, in -> String.valueOf(in)));

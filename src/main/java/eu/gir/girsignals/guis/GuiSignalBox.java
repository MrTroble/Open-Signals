...
public class GuiSignalBox extends GuiBase {
	...
	private void selectLink(final UIEntity parent, final SignalBoxNode node, final PathOptionEntry option, final ImmutableSet<Entry<BlockPos, LinkType>> entrySet, final LinkType type, final PathEntryType<BlockPos> entryType) {
		...
		if (!positions.isEmpty()) {
			...
			blockSelect.findRecursive(UIEnumerable.class).forEach(e -> {
				e.setMin(-1);
				final int index = option.getEntry(entryType).map(entry -> positions.indexOf(entry)).orElse(-1);
				e.setIndex(index);
				e.setID(null);
			});
	...
	private void setupModeSettings(final UIEntity parent, final EnumGuiMode mode, final Rotation rotation, final SignalBoxNode node, final PathOptionEntry option) {
		...
		case CORNER:
		case STRAIGHT: {
			final EnumPathUsage path = option.getEntry(PathEntryType.PATHUSAGE).orElse(EnumPathUsage.FREE);
			...
			final int speed = option.getEntry(PathEntryType.SPEED).filter(n -> n < 16).orElse(Integer.MAX_VALUE);
	...
	private void initializePageTileConfig(final SignalBoxNode node) {
		...
		node.forEach(modeSet -> setupModeSettings(list, modeSet.mode, modeSet.rotation, node, node.getOption(modeSet).get()));
	...
	private void sort(final SortOptions sort) {
		...
		case NAME_DESCENIDNG:
			break;
		case TYPE_ASSANDING:
			settingslist.sort((o1, o2) -> Integer.compare(o1.getValue().ordinal(), o2.getValue().ordinal()));
			break;

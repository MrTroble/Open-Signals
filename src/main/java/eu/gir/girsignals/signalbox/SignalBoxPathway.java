...
public class SignalBoxPathway implements INetworkSavable {
	...
	public SignalBoxPathway(final Map<Point, SignalBoxNode> modeGrid, final List<SignalBoxNode> pNodes, final PathType type) {
		this(modeGrid);
	...
	private void initalize() {
		...
		foreachEntry((optionEntry, node) -> {
			...
			optionEntry.getEntry(PathEntryType.BLOCKING).ifPresent(position -> mapOfBlockingPositions.put(position, node));
			optionEntry.getEntry(PathEntryType.RESETING).ifPresent(position -> mapOfResetPositions.put(position, node));
		});
		final SignalBoxNode firstNode = this.listOfNodes.get(this.listOfNodes.size() - 1);
		this.firstPoint = firstNode.getPoint();
		final BlockPos firstPos = makeFromNext(type, firstNode, this.listOfNodes.get(this.listOfNodes.size() - 2), Rotation.NONE);
		final SignalBoxNode lastNode = this.listOfNodes.get(0);
		this.lastPoint = lastNode.getPoint();
		final BlockPos lastPos = makeFromNext(type, lastNode, this.listOfNodes.get(1), Rotation.CLOCKWISE_180);
	...
	private BlockPos makeFromNext(final PathType type, final SignalBoxNode first, final SignalBoxNode next, final Rotation pRotation) {
		...
		for (final EnumGuiMode mode : type.getModes()) {
			final BlockPos possiblePosition = first.getOption(new ModeSet(mode, rotation)).flatMap(option -> option.getEntry(PathEntryType.SIGNAL)).orElse(null);
	...
	private void foreachEntry(final Consumer<PathOptionEntry> consumer, final @Nullable Point point) {
	...
	private void foreachEntry(final BiConsumer<PathOptionEntry, SignalBoxNode> consumer, final @Nullable Point point) {
		for (int i = 1; i < listOfNodes.size() - 1; i++) {
			...
			if (current.getPoint().equals(point))
				break;
			current.getOption(new Path(oldPos, newPos)).ifPresent(entry -> consumer.accept(entry, current));
		}
	}
	
	public void setPathStatus(final EnumPathUsage status, final @Nullable Point point) {
		foreachEntry(option -> {
			option.getEntry(PathEntryType.OUTPUT).ifPresent(pos -> loadOps.setPower(pos, !status.equals(EnumPathUsage.FREE)));
	...
	public void updatePathwaySignals() {
		this.signalPositions.ifPresent(entry -> loadOps.loadAndConfig(speed, entry.getKey(), entry.getValue()));
	}
	...
	@Override
	public int hashCode() {
		return Objects.hash(firstPoint, lastPoint, listOfNodes, mapOfBlockingPositions, mapOfResetPositions, speed, type);
	}
	...
	public boolean equals(final Object obj) {
		...
		return Objects.equals(firstPoint, other.firstPoint) && Objects.equals(lastPoint, other.lastPoint) && Objects.equals(listOfNodes, other.listOfNodes) && Objects.equals(mapOfBlockingPositions, other.mapOfBlockingPositions) && Objects.equals(mapOfResetPositions, other.mapOfResetPositions) && speed == other.speed && type == other.type;
	}
	
	@Override
	public String toString() {
		return "SignalBoxPathway [type=" + type + ", speed=" + speed + ", signalPositions=" + signalPositions + ", mapOfResetPositions=" + mapOfResetPositions + ", mapOfBlockingPositions=" + mapOfBlockingPositions + ", firstPoint=" + firstPoint + ", lastPoint=" + lastPoint + ", list=" + listOfNodes + "]";
	}

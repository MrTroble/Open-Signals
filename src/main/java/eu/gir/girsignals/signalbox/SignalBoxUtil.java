...
public final class SignalBoxUtil {
	...
	public static Optional<SignalBoxPathway> requestWay(final Map<Point, SignalBoxNode> modeGrid, final Point p1, final Point p2) {
		...
		while (!openList.isEmpty()) {
			...
			for (final Path entry : cSNode.connections()) {
				...
				checker.path = checker.previous == null || neighbour == null ? null : new Path(checker.previous, neighbour);
				if (checker.check()) {
	...
	public static void drawTextured(final UIEntity entity, final int textureID) {
		...
		bufferbuilder.pos(entity.getWidth(), entity.getHeight(), textureID).tex(offset + 0.25, 0.5).endVertex();
	...
	public static void drawLines(final int x1, final int x2, final int y1, final int y2, final int color) {
		final float f3 = (color >> 24 & 255) / 255.0F;
		final float f = (color >> 16 & 255) / 255.0F;
		final float f1 = (color >> 8 & 255) / 255.0F;
		final float f2 = (color & 255) / 255.0F;

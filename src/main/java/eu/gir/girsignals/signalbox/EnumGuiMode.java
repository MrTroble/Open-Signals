...
public enum EnumGuiMode {
	...
	CORNER(0, 0.5, 0.5, 1),
	END(1, 0.30, 1, 0.70),
	PLATFORM((parent, color) -> SignalBoxUtil.drawRect(0, 0, ((UIEntity)parent).getWidth(), ((UIEntity)parent).getHeight() / 3, color)),
	BUE((parent, color) -> {
	...
	private EnumGuiMode(double x1, double y1, double x2, double y2) {
		this.consumer = (parent, color) -> SignalBoxUtil.drawLines((int) (x1 * ((UIEntity)parent).getWidth()), (int) (x2 * ((UIEntity)parent).getWidth()), (int) (y1 * ((UIEntity)parent).getHeight()), (int) (y2 * ((UIEntity)parent).getHeight()), color);
	}

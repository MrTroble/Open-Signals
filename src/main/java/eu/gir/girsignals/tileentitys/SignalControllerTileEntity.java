...
public class SignalControllerTileEntity extends TileEntity
    ...
    public void onLink() {
        new Thread(() -> {
            loadChunkAndGetTile((sigtile, ch) -> {
                ...
                listOfSupportedIndicies = supportedSignaleStates.values().stream()
                        .mapToInt(Integer::intValue).toArray();
                tableOfSupportedSignalTypes = supportedSignaleStates;
                signalTypeCache = ((Signal) ch.getBlockState(linkedSignalPosition).getBlock())
                        .getID();
        ...
        try {
            Callable<Boolean> call = () -> {
                ...
                boolean flag2 = entity instanceof SignalTileEnity
                        && ((SignalTileEnity) entity).getBlockID() != -1;
                if (flag2) {
    ...
    private void updateRSProfiles() {
        ...
        compound.getKeySet().stream().filter(key -> key.startsWith("enabled.")).forEach(e -> {
            final String[] args = e.split("\\.");
            final int state = compound.getInteger(e);
            if (state != EnumState.DISABLED.ordinal()) {
                final EnumFacing face = EnumFacing.valueOf(args[2].toUpperCase());
                if (!statesEnabled.containsKey(face))
                    statesEnabled.put(face, Maps.newHashMap());
                statesEnabled.get(face).put(EnumState.values()[state], args[1]);
            }
        });
    }

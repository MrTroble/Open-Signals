...
public class SignalTram extends Signal {
	...
	public static final SEProperty<TRAMTYPE> TRAMSIGNAL_TYPE = SEProperty.of("signaltramtype", TRAMTYPE.TRAM,
			ChangeableStage.GUISTAGE, false);
	public static final SEProperty<TRAM> TRAMSIGNAL = SEProperty.of("signaltram", TRAM.OFF,
			ChangeableStage.APISTAGE_NONE_CONFIG, true, t -> t.entrySet().stream()
					.anyMatch((e -> e.getKey().equals(TRAMSIGNAL_TYPE) && e.getValue().equals(TRAMTYPE.TRAM))));
	public static final SEProperty<CAR> CARSIGNAL = SEProperty.of("signalcar", CAR.OFF,
			ChangeableStage.APISTAGE_NONE_CONFIG, false, t -> t.entrySet().stream()
					.anyMatch((e -> e.getKey().equals(TRAMSIGNAL_TYPE) && e.getValue().equals(TRAMTYPE.CAR))));
	public static final SEProperty<PED> PEDSIGNAL = SEProperty.of("signalped", PED.OFF,
			ChangeableStage.APISTAGE_NONE_CONFIG, false, t -> t.entrySet().stream()
					.anyMatch((e -> e.getKey().equals(TRAMSIGNAL_TYPE) && e.getValue().equals(TRAMTYPE.PEDESTRIAN))));
}

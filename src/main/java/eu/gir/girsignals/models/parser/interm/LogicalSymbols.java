...
public enum LogicalSymbols {
	...
	AND("&&", IntermidiateAnd::new), OR("||", IntermidiateOr::new), NEGATE("!", IntermidiateNegate::new);
	...
	public static LogicalSymbols find(final String name) {
		return Arrays.stream(LogicalSymbols.values()).filter(symbol -> symbol.symbol.equals(name)).findAny().orElse(null);
	}
}
